package kz.bcc.card_scanner.scanner_core.models
...
data class CardScannerOptions(
    ...
    constructor(configMap: Map<String, String>) : this(
        ...
        validCardsToScanBeforeFinishingScan = configMap[ParcelKeys.validCardsToScanBeforeFinishingScan.value]?.toInt()
            ?: 11,
        cardHolderNameBlackListedWords = ArrayList(configMap[ParcelKeys.cardHolderNameBlackListedWords.value]?.split(',') ?: listOf()),
        ...
        possibleCardHolderNamePositions = ArrayList(configMap[ParcelKeys.possibleCardHolderNamePositions.value]?.split(',') ?: listOf(CardHolderNameScanPositions.belowCardNumber.value))
    )
